...
from transformers import AutoModelForCausalLM, ApertusConfig, ApertusForCausalLM, GenerationConfig
...
def add_arguments(parser):
    group = parser.add_argument_group(title="Apertus HF saver.")
    group.add_argument(
        "--hf-tokenizer",
        type=str,
        default=None,
...
def save_checkpoint(queue: mp.Queue, args):
    ...
    if md.checkpoint_args.position_embedding_type != "rope":
        raise ValueError("Apertus model must use RoPE")
    if not md.checkpoint_args.use_rope_scaling:
        raise ValueError("Apertus model must use llama3 RoPE scaling")
    if md.checkpoint_args.normalization != "RMSNorm":
        raise ValueError("Apertus model must not use mlp bias")
    if md.checkpoint_args.add_bias_linear:
        raise ValueError("Apertus model must use RMSNorm")
    ...
    with TemporaryDirectory(prefix=str(save_dir/"tmp")) as tmp_save_dir:
        ...
        if args.hf_tokenizer:
            ...
            conf.bos_token_id = tokenizer.bos_token_id

        # Prepare qkv slices.
        head_size = conf.hidden_size // conf.num_attention_heads
        heads_per_group = conf.num_attention_heads // conf.num_key_value_heads
        qkv_total_heads = conf.num_attention_heads + 2 * conf.num_key_value_heads
        q_slice = torch.cat(
            [
                torch.arange(
                    (heads_per_group + 2) * i,
                ...
                for i in range(conf.num_key_value_heads)
            ]
        )
        ...
        conf.save_pretrained(tmp_save_dir)
        ...
        for i_layer in range(conf.num_hidden_layers):
            ...
            qkv_weights = qkv_weights.reshape(
                [qkv_total_heads, head_size, conf.hidden_size]
            )
            state_dict |= {
                f"model.layers.{i_layer}.self_attn.q_proj.weight": qkv_weights[
                    q_slice
                ].reshape(-1, conf.hidden_size),

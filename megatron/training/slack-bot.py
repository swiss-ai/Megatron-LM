...
class SlackBot:
    def __init__(
        ...
        slack_token: str,
        channel_id: str,
        status_interval: int = 2,  # hours between regular updates
        ...
        Args:
            ...
            - urgent_thresholds: Dictionary of metric names and their threshold values
              that should trigger alert notifications 🚨
        """
        ...
        self.urgent_thresholds = urgent_thresholds or {
            "loss": 3.0,  # Alert if loss exceeds this
            "gradient_norm": 5.0,  # 
            "throughput": 5.0,  # (tok/sec) 
        }
    ...
    def check_urgent_conditions(self, metrics: Dict[str, float]) -> List[str]:
        """Check if any metrics exceed their urgent thresholds"""
        urgent_messages = []
    ...
    def update(self, metrics: Dict[str, float]):
        ...
        # Regular update (if enough time has passed) + eventual Alerts 
        ...
        # Alerts can be sent even before regular update times
...
if __name__ == "__main__":
    ...
    monitor = SlackBot(
        ...
        status_interval=10/3600,  # Status update every 10s
        urgent_thresholds={
            "loss": 3.0,
            "gradient_norm": 5.0,
            "throughput": 10.0
    ...
    for _ in range(8): 
        metrics = {
            "loss": np.random.uniform(1.5, 4.0),
            "gradient_norm": np.random.uniform(1.0, 6.0),
            "throughput": np.random.uniform(5.0, 50.0)
        }
        monitor.update(metrics)
        time.sleep(10)  # In our case, this will be replaced with actual training

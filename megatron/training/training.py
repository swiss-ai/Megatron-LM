...
def get_megatron_optimizer_config(args: Any) -> OptimizerConfig:
    ...
    if args.decoupled_lr is not None:
        decoupled_lr_config = {"max_lr": args.decoupled_lr}
...
def setup_model_and_optimizer(
    ...
    if (
        ...
        args.iteration, args.num_floating_point_operations_so_far, args.tokens_so_far = load_checkpoint(
                ...
                skip_load_to_model_and_opt=HAVE_FSDP2 and getattr(args, "use_torch_fsdp2", False) and args.ckpt_format == "torch_dist",strict=False)
...
def get_train_valid_test_num_samples():
    ...
    if args.phase_transition_iterations:
        last_transition = max(t for t in [0] + args.phase_transition_iterations if t <= args.iteration)
        train_samples_in_current_phase = train_samples - last_transition * args.global_batch_size
    else:

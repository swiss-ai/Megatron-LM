...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def _forward_attention(
        ...
        *,
        ...
        self.recompute_input_layernorm = self.recompute_input_layernorm and not self.config.post_layer_norm  # no recompute if post layer norm is enabled (Dhia: TODO)
        # Self attention.
        ...
        if self.recompute_input_layernorm:   # using pre-norm for sure
        ...
        # Optional: Layer norm post the cross-attention.
        if self.config.post_layer_norm:   # no recomputation if post layer norm is enabled (DHIA: TODO --> add support for recompute when using post layer norm OR add a check in arguments.py not here)
            self.recompute_pre_mlp_layernorm = False
            pre_mlp_layernorm_output = hidden_states  # Skip pre-layernorm
        elif self.recompute_pre_mlp_layernorm:  # we are now sure we use pre-norm
    ...
    def _forward_mlp(self, pre_mlp_layernorm_output, residual, inference_context=None):
        ...
        should_chunk_mlp_for_prefill = (
            ...
            and not self.config.post_layer_norm  # disable chunking with post-layernorm (Dhia: TODO)
        )

        # Disable MLP recomputation with post-norm (Dhia: TODO)
        self.recompute_mlp = self.recompute_mlp and not self.config.post_layer_norm

        if self.recompute_mlp:   # surely we are using pre-norm
            if self.config.fp8:
                # import here to avoid circular import
        ...
        elif should_chunk_mlp_for_prefill:      # surely using pre-norm
        ...
        else:  # no recomputation, no chunking
            mlp_output, bias = self.mlp(pre_mlp_layernorm_output)   # either pre-norm or post-norm
            if self.config.post_layer_norm:
